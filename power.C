...
unsigned long  LockID[3];//={0x20343141,0x3830430E,0x00038001};
unsigned long  TempLockID[3];//={0x20343141,0x3830430E,0x00038001};
...
void test()
{
    ...
    for (i=0; i <8; i++){
    	...
    	LockID[0]=((LockID[0]>>(3*i))+j*k*m);

    	LockID[1]=((LockID[1]>>(3*i))+j*k*m);

    	LockID[2]=((LockID[2])>>(3*i)+j*k*m);
