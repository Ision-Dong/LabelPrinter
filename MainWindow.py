...
from utils.history import LineEditWithHistory
...
class Ui_MainWindow(QMainWindow):
    ...
    def setupUi(self, MainWindow):
        ...
        self.message.signal.connect(self.alert_message)
        ...
        self.lineEdit_7 = LineEditWithHistory(self.centralwidget)
    ...
    def menushow(self, pos):
        ...
        if self.tableWidget.item(row, column) in self.tableWidget.selectedItems():
            ...
            re_name = menu.addAction("Upload")
            re_name.setIcon(qtawesome.icon("fa5s.upload", color='gray'))
            clear_color = menu.addAction("Clear")
            clear_color.setIcon(qtawesome.icon("mdi.format-clear", color='gray'))
    ...
    def __clear(self):
        cur = DBConnector()
        msn = self.tableWidget.item(self.tableWidget.currentRow(), 2).text()
        cur.update(sn=msn, column="IS_UPLOAD", column_value=0)
    ...
    def upload(self):
        cur = DBConnector()
        msn = self.tableWidget.item(self.tableWidget.currentRow(), 2).text()
        cur.update(sn=msn, column="IS_UPLOAD", column_value=1)
        item = self.tableWidget.item(self.tableWidget.currentRow(), 2)
    ...
    def __delete(self):
        sn = self.tableWidget.item(self.tableWidget.currentRow(), 2).text()
        cursor = DBConnector()
        cursor.delete(sn)
        cursor.close()
    ...
    def authorize(self):
        try:
            ...
            self.error_message = "Com Port not ready!!!"
            self.message.start()
            return
        ...
        try:
            ...
            for count, command in enumerate(commands):
                ...
                try:
                    ...
                    return_ = " ".join(back.split(" ")[5:-1]) if not DEBUG else " ".join(back.split(" "))
                    ...
                    if check_sum.upper() != last_check_sum: # 如果check sum 不相等，弹出窗口。程序终止
                        self.error_message = " CheckSum check failure!"
                        self.message.start()
                        return
            ...
            if len(return_data["MODEL_CODE"]) != 13:
                self.error_message = "The lenght of MODEL CODE must be 13!"
                self.message.start()
                return
    ...
    def alert_message(self, message=None):
        messageBox = QMessageBox(QMessageBox.Icon(3), "Error", message or self.error_message, QMessageBox.Ok, self)
        messageBox.button(QMessageBox.Ok).setText("yes")
        reply = messageBox.exec()
        from utils import log
        if reply == QMessageBox.Ok:
            log.output(message=message or self.error_message)
    ...
    def view_datas(self, headers=None):
        ...
        columns.pop(columns.index("IS_UPLOAD"))
        self.tableWidget.setSelectionBehavior(QAbstractItemView.SelectRows)
        self.tableWidget.setEditTriggers(QAbstractItemView.NoEditTriggers)
        cur.select(skip="IS_UPLOAD")
        datas = []
        for d in cur.datas:
            datas.append(d)
        ...
        for index, d in enumerate(datas):
            for in_, v in enumerate(d):
                ...
                if in_ == 2 and cur.is_upload(v):
                    cell.setBackground(QColor("green"))
                self.tableWidget.setItem(index, in_, cell)
